...
function processSitemap (options, document, callback) {
  ...
  smLib.parse(sitemapOptions, document, function (err) {
    delete options.outputPath;
    var outputPath = base.outputFile(options, options.source);

    if (!err && options.sitemapOutputDir) {
      ...
      _.forEach(
        ...
        function (sitemapNode) {
          ...
          var shouldProcess = !options.sitemapPolicy ||
            !smLib.stillCurrent(sitemapNode, options);
