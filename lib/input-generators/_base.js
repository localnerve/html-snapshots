  ...
  if (typeof obj !== "function") {
    if (typeof obj !== "undefined") {
      if (Object.prototype.toString.call(obj) !== "[object Object]") {
        ...
        result = (key, passthru) => {
          if (obj[key] === void 0) {
            return obj.__default || (passthru ? key : undefined);
          } else {
            return obj[key];
          }
        };
...
function prepOptions (options, listener) {
  ...
  [
    "selector", "timeout", "useJQuery", "verbose", "phantomjsOptions", "debug"
  ]
...
function getOutputPath (options, page, parse) {
  const pagePart = urlm.parse(page);
...
function mapOutputFile (options, page, parse) {
  ...
  const outputPath = getOutputPath(options, page, parse);
...
module.exports = {
  ...
  input: function (options, page) {
    const parse = {};
    const outputFile = mapOutputFile(options, page, parse);
    ...
    if (outputFile) {
      ...
      options._inputEmitter.emit("input", {
        ...
        // map the input page to a debug flag
        debug: options.debug(page),
