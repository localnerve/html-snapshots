    ...
    if (!err) {
      ...
      _.find(
        ...
        function(urlNode) {
          ...
          var process = !options.sitemapPolicy || !stillCurrent(urlNode, options);
...
function getSitemapUrl(options, callback) {
  var request = http.get(options.source, function(res) {
    var responseBody = "";
    var badType = res.headers["content-type"] && !~res.headers["content-type"].indexOf("xml");

    if (!(res.statusCode >= 200 && res.statusCode < 300)) {
      callback("'"+options.source+"' status: '"+res.statusCode+"', GET failed.");
    } else if (badType) {
      callback("'"+options.source+"' not xml, GET failed.");
    } else {
