...
function Notifier() {
   ...
   */
  this.start = function(pollInterval, listener) {
    var result = (pollInterval > 0 && typeof listener === "function");
    if (result) {
      callback = listener;
      ...
      watcher = setInterval(function(self) {
        for (var file in files) {
          if (fs.existsSync(file)) {
            self.remove(file);
            //console.log("WATCH remove: file="+file);            
          }
        }
      ...
      console.error("Bad poll interval or async listener supplied");
    }
    return result;
  };
