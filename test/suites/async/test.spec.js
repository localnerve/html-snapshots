...
const asyncLocal = require("../../../lib/async");
...
describe("async", function () {
  ...
  describe("Notifier", function () {
    ...
    it("should fail if negative poll interval supplied", function () {
      const result = notifier.start(-1, mockInput, function (err) {
    ...
    it("should fail if zero poll interval supplied", function () {
      const result = notifier.start(0, mockInput, function (err){
        assert.fail(err, "[not undefined]", "should never have been called", "?");
      });
    ...
    it("should fail if no input generator is supplied", function () {
      const result = notifier.start(250, undefined, function (err) {
        assert.fail(err, "[not undefined]", "should never have been called", "?");
      });
    ...
    function (done) {
      ...
      const q = asyncLib.queue(function (task, callback) {
        task(callback);
      }, 1);
